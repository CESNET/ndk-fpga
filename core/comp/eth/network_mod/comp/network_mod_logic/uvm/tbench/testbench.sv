...
module testbench;
    ...
    ) user_mfb_rx              (CLK_USER);
    mfb_if #(
        .REGIONS     (CORE_REGIONS),
        .REGION_SIZE (CORE_REGION_SIZE),
        .BLOCK_SIZE  (BLOCK_SIZE),
        .ITEM_WIDTH  (ITEM_WIDTH),
        .META_WIDTH  (0)
    ) core_mfb_tx[ETH_CHANNELS](CLK_CORE);
    // RX path
    mfb_if #(
        .REGIONS     (CORE_REGIONS),
        .REGION_SIZE (CORE_REGION_SIZE),
        ...
        .ITEM_WIDTH  (ITEM_WIDTH),
        .META_WIDTH  (0)
    ) core_mfb_rx[ETH_CHANNELS](CLK_CORE);
    mfb_if #(
        .REGIONS     (USER_REGIONS),
        .REGION_SIZE (USER_REGION_SIZE),
        .BLOCK_SIZE  (BLOCK_SIZE),
        .ITEM_WIDTH  (ITEM_WIDTH),
        .META_WIDTH  (0)
    ) user_mfb_tx              (CLK_USER);
    mvb_if #(
        .ITEMS      (USER_REGIONS),
        .ITEM_WIDTH (USER_MVB_WIDTH)
    ) user_mvb_tx              (CLK_USER);
    // MVB discard (monitors discard of RX MAC Lites)
    mvb_if #(
        .ITEMS      (RX_MAC_LITE_REGIONS),
        .ITEM_WIDTH (1)
    ) mvb_discard[ETH_CHANNELS](CLK_CORE);
    // MI
    mi_if #(
    ...
    initial begin
        ...
        automatic virtual mfb_if #(
            ...
            .ITEM_WIDTH  (ITEM_WIDTH),
            .META_WIDTH  (0)
        ) v_core_mfb_tx[ETH_CHANNELS] = core_mfb_tx;
        ...
        uvm_config_db#(virtual mfb_if #(
            ...
            .ITEM_WIDTH  (ITEM_WIDTH),
            .META_WIDTH  (META_WIDTH)
        ))::set(null, "", "vif_user_rx_mfb", user_mfb_rx);
        for (int unsigned i = 0; i < ETH_CHANNELS; i++ ) begin
            ...
            uvm_config_db#(virtual mfb_if #(
                ...
                .ITEM_WIDTH  (ITEM_WIDTH),
                .META_WIDTH  (0)
            ))::set(null, "", {"vif_core_tx_mfb_",i_string}, v_core_mfb_tx[i]);
        end
        ...
        for (int unsigned i = 0; i < ETH_CHANNELS; i++ ) begin
            ...
            uvm_config_db#(virtual mfb_if #(
                ...
                .ITEM_WIDTH  (ITEM_WIDTH),
                .META_WIDTH  (0)
            ))::set(null, "", {"vif_core_rx_mfb_",i_string}, v_core_mfb_rx[i]);
        end
        uvm_config_db#(virtual mfb_if #(
            .REGIONS     (USER_REGIONS),
            .REGION_SIZE (USER_REGION_SIZE),
            ...
            .ITEM_WIDTH  (ITEM_WIDTH),
            .META_WIDTH  (0)
        ))::set(null, "", "vif_user_tx_mfb", user_mfb_tx);
        uvm_config_db#(virtual mvb_if #(
            .ITEMS      (USER_REGIONS),
            .ITEM_WIDTH (USER_MVB_WIDTH)
        ))::set(null, "", "vif_user_tx_mvb", user_mvb_tx);
        ...
        for (int unsigned i = 0; i < ETH_CHANNELS; i++ ) begin
            ...
            uvm_config_db#(virtual mvb_if #(
                .ITEMS      (RX_MAC_LITE_REGIONS),
                .ITEM_WIDTH (1)
            ))::set(null, "", {"vif_mvb_discard_",i_string}, v_mvb_discard[i]);
        end
        ...
        uvm_config_db#(virtual mi_if #(
            .DATA_WIDTH (MI_DATA_WIDTH),
            .ADDR_WIDTH (MI_ADDR_WIDTH)
        ))::set(null, "", "MI_INTERFACE", mi_config);
        // --------------------------------------------------------------------
