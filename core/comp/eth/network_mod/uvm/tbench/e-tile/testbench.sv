...
module testbench;
    ...
    avst_if #(
        ...
        .ITEM_WIDTH  (ITEM_WIDTH),
        .META_WIDTH  (6)
    ) eth_rx[ETH_PORTS] (CLK_ETH[0]);
    avst_if #(
        .REGIONS     (ETH_PORT_CHAN[0]),
        .REGION_SIZE (REGION_SIZE * BLOCK_SIZE),
        .ITEM_WIDTH  (ITEM_WIDTH),
        .META_WIDTH  (1)
    ) eth_tx[ETH_PORTS] (CLK_ETH[0]);
    ...
    initial begin
        ...
        automatic virtual avst_if #(
            ...
            .ITEM_WIDTH  (ITEM_WIDTH),
            .META_WIDTH  (6)
        )                vif_eth_rx     [ETH_PORTS] = eth_rx;
        automatic virtual avst_if #(
            .REGIONS     (ETH_PORT_CHAN[0]),
            .REGION_SIZE (REGION_SIZE * BLOCK_SIZE),
            .ITEM_WIDTH  (ITEM_WIDTH),
            .META_WIDTH  (1)
        )                vif_eth_tx     [ETH_PORTS] = eth_tx;
        ...
        for (int unsigned it = 0; it < ETH_PORTS; it++) begin
            ...
            uvm_config_db#(virtual avst_if #(
                ...
                .ITEM_WIDTH  (ITEM_WIDTH),
                .META_WIDTH  (6)
            ))::set(null, "", $sformatf("vif_eth_rx_%0d", it) , vif_eth_rx[it]);
            uvm_config_db#(virtual avst_if #(
                .REGIONS     (ETH_PORT_CHAN[0]),
                .REGION_SIZE (REGION_SIZE * BLOCK_SIZE),
                .ITEM_WIDTH  (ITEM_WIDTH),
                .META_WIDTH  (1)
            ))::set(null, "", $sformatf("vif_eth_tx_%0d", it) , vif_eth_tx[it]);
        end
