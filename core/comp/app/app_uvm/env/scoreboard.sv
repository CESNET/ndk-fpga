...
class scoreboard #(
    ...
    `ndk_component_param_utils(uvm_app_core::scoreboard #(ETH_STREAMS, ETH_RX_HDR_WIDTH, ETH_TX_HDR_WIDTH, DMA_STREAMS,
                                   DMA_RX_CHANNELS, DMA_TX_CHANNELS, DMA_HDR_META_WIDTH, DMA_PKT_MTU, ITEM_WIDTH),
        $sformatf("uvm_app_core::scoreboard#(%0d,%0d,%0d,%0d,%0d,%0d,%0d,%0d,%0d)",
            // verilog_lint: waive line-length
            ETH_STREAMS, ETH_RX_HDR_WIDTH, ETH_TX_HDR_WIDTH, DMA_STREAMS, DMA_RX_CHANNELS, DMA_TX_CHANNELS, DMA_HDR_META_WIDTH, DMA_PKT_MTU, ITEM_WIDTH
        )
    )

    //RESET --
    typedef scoreboard #(ETH_STREAMS, ETH_RX_HDR_WIDTH, ETH_TX_HDR_WIDTH, DMA_STREAMS, DMA_RX_CHANNELS, DMA_TX_CHANNELS,
                        DMA_HDR_META_WIDTH, DMA_PKT_MTU, ITEM_WIDTH) this_type;
    ...
    uvm_analysis_export #(uvm_app_core_top_agent::sequence_eth_item#(
    ...
    uvm_analysis_export #(uvm_app_core_top_agent::sequence_dma_item#(
        DMA_RX_CHANNELS,
        $clog2(DMA_PKT_MTU+1),
        DMA_HDR_META_WIDTH,
        ITEM_WIDTH
    ...
    function model #(ETH_STREAMS, ETH_RX_HDR_WIDTH, DMA_STREAMS, DMA_RX_CHANNELS, DMA_TX_CHANNELS, DMA_HDR_META_WIDTH,
                    DMA_PKT_MTU, ITEM_WIDTH) model_get();
        return m_model;
    endfunction
    ...
    function void build_phase(uvm_phase phase);
        ...
        for (int unsigned it = 0; it < ETH_STREAMS; it++) begin
            ...
            if (cfg.compare_eth == config_item::CMP_ORDERED) begin
                eth_mvb_cmp[it] = scoreboard_channel_header_ordered#(
                    0,
                    2**ETH_TX_CHANNEL_WIDTH,
                    2**ETH_TX_LENGTH_WIDTH-1,
                    ITEM_WIDTH
                )::type_id::create({"eth_mvb_cmp_", it_num}, this);
                eth_mfb_cmp[it] = scoreboard_channel_mfb_ordered   #(
                    0,
                    2**ETH_TX_CHANNEL_WIDTH,
                    2**ETH_TX_LENGTH_WIDTH-1,
                    ITEM_WIDTH
                )::type_id::create({"eth_mfb_cmp_", it_num}, this);
            end else if (cfg.compare_eth == config_item::CMP_UNORDERED) begin
                eth_mvb_cmp[it] = scoreboard_channel_header_unordered#(
                    0,
                    2**ETH_TX_CHANNEL_WIDTH,
                    2**ETH_TX_LENGTH_WIDTH-1,
                    ITEM_WIDTH
                )::type_id::create({"eth_mvb_cmp_", it_num}, this);
                eth_mfb_cmp[it] = scoreboard_channel_mfb_unordered   #(
                    0,
                    2**ETH_TX_CHANNEL_WIDTH,
                    2**ETH_TX_LENGTH_WIDTH-1,
                    ITEM_WIDTH
                ...
                eth_mvb_cmp[it] = scoreboard_channel_header_tagged#(
                    0,
                    2**ETH_TX_CHANNEL_WIDTH,
                    2**ETH_TX_LENGTH_WIDTH-1,
                    ITEM_WIDTH
                )::type_id::create({"eth_mvb_cmp_", it_num}, this);
                eth_mfb_cmp[it] = scoreboard_channel_mfb_tagged   #(
                    0,
                    2**ETH_TX_CHANNEL_WIDTH,
                    2**ETH_TX_LENGTH_WIDTH-1,
                    ITEM_WIDTH
        ...
        for (int unsigned it = 0; it < DMA_STREAMS; it++) begin
            ...
            if (cfg.compare_dma == config_item::CMP_ORDERED) begin
                dma_mvb_cmp[it] = scoreboard_channel_header_ordered#(
                    DMA_HDR_META_WIDTH,
                    DMA_TX_CHANNELS,
                    DMA_PKT_MTU,
                    ITEM_WIDTH
                )::type_id::create({"dma_mvb_cmp_", it_num}, this);
                dma_mfb_cmp[it] = scoreboard_channel_mfb_ordered   #(
                    DMA_HDR_META_WIDTH,
                    DMA_TX_CHANNELS,
                    DMA_PKT_MTU,
                    ITEM_WIDTH
                )::type_id::create({"dma_mfb_cmp_", it_num}, this);
            end  else if (cfg.compare_dma == config_item::CMP_UNORDERED) begin
                dma_mvb_cmp[it] = scoreboard_channel_header_unordered#(
                    DMA_HDR_META_WIDTH,
                    DMA_TX_CHANNELS,
                    DMA_PKT_MTU,
                    ITEM_WIDTH
                )::type_id::create({"dma_mvb_cmp_", it_num}, this);
                dma_mfb_cmp[it] = scoreboard_channel_mfb_unordered   #(
                    DMA_HDR_META_WIDTH,
                    DMA_TX_CHANNELS,
                    DMA_PKT_MTU,
                    ITEM_WIDTH
                ...
                dma_mvb_cmp[it] = scoreboard_channel_header_tagged#(
                    DMA_HDR_META_WIDTH,
                    DMA_TX_CHANNELS,
                    DMA_PKT_MTU,
                    ITEM_WIDTH
                )::type_id::create({"dma_mvb_cmp_", it_num}, this);
                dma_mfb_cmp[it] = scoreboard_channel_mfb_tagged   #(
                    DMA_HDR_META_WIDTH,
                    DMA_TX_CHANNELS,
                    DMA_PKT_MTU,
                    ITEM_WIDTH
        ...
        m_model = model #(ETH_STREAMS, ETH_RX_HDR_WIDTH, DMA_STREAMS, DMA_RX_CHANNELS, DMA_TX_CHANNELS,
                         DMA_HDR_META_WIDTH, DMA_PKT_MTU, ITEM_WIDTH)::type_id::create("m_model", this);
    endfunction
