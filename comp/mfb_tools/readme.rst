...
.. _mfb-design-considerations:

Design considerations
---------------------
When designing a module with MFB interfaces, some handshaking schemes can
...
5. The above rules apply mainly to a component's input and output MFB
...
.. note:: The behavior described in Rule#3 can be achieved in a couple of ways.
          ...
          `DST_RDY=1 or SRC_RDY=0`, an :ref:`(MFB) PIPE <mfb_pipe>` with generic
          `USE_DST_RDY=True`, an :ref:`(MFB) FIFO <mfb_fifox>`, etc. However, it
          would be useful for optimization purposes if there were an option to
          use just a simple, DST_RDY-enabled output register. The usage of such
          goes against this rule but a wise user can utilize it for better
          performance (meeting the timing constraints). Due to this reason, it
