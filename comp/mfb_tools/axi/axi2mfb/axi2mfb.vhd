...
begin
    ...
    assert (is_powerof2(REGION_SIZE))
        report "AXI2MFB: REGION_SIZE is not power of 2"
        severity FAILURE;

    -- 3. MFB BLOCK_SIZE is expected to be 2^n
    assert (is_powerof2(BLOCK_SIZE))
        report "AXI2MFB: BLOCK_SIZE is not power of 2"
        severity FAILURE;

    -- 4. MFB and AXI stream data width needs to be equal
    assert (MFB_DATA_WIDTH = AXI_DATA_WIDTH)
        report "AXI2MFB: MFB and AXIs data width is not matching"
        severity FAILURE;

    -- 5. MFB and AXI meta signal must have same width
    assert (META_WIDTH = AXI_USER_WIDTH)
        report "AXI2MFB: MFB and AXIs meta width is not matching"
        severity FAILURE;
    ...
    -- psl assert_tkeep :
    --      assert always (RX_AXI_TVALID = '1' and OR(RX_AXI_TKEEP) /= '0') abort (RST) @rising_edge(CLK)
    ...
    begin
        if (rising_edge(CLK)) then
            if (RX_AXI_TVALID = '1') then
                ...
                assert (ms_one_index = ls_zero_index)
                    report "AXI2MFB: TKEEP unsupported format while RX_AXI_TVALID == 1"
                    severity FAILURE;
            end if;
        end if;
    end process;
