...
class virtual_sequence_base #(int unsigned MFB_REGIONS, int unsigned MFB_REGION_SIZE, int unsigned MFB_BLOCK_SIZE, int unsigned MFB_ITEM_WIDTH, int unsigned PKT_MTU, int unsigned USERMETA_WIDTH, int unsigned RX_MVB_ITEM_WIDTH) extends uvm_sequence;
    ...
    uvm_mfb::sequence_lib_tx             #(MFB_REGIONS, MFB_REGION_SIZE, MFB_BLOCK_SIZE, MFB_ITEM_WIDTH, USERMETA_WIDTH) m_tx_mfb;
    uvm_mvb::sequence_lib_tx             #(MFB_REGIONS, USERMETA_WIDTH)                                                  m_tx_mvb;
    ...
    virtual function void init();
        ...
        m_rx_mfb.min_random_count = 100;
        m_rx_mfb.max_random_count = 150;

        // Create the RX MFB state config
        m_rx_mfb_config = new("m_rx_mfb_config");
        m_rx_mfb_config.transactions_min = 150;
        m_rx_mfb_config.transactions_max = 200;
        assert(m_rx_mfb_config.randomize());
        uvm_config_db #(uvm_common::sequence_cfg)::set(p_sequencer.m_rx_mfb, "", "state", m_rx_mfb_config);
        ...
        m_tx_mfb = uvm_mfb::sequence_lib_tx #(MFB_REGIONS, MFB_REGION_SIZE, MFB_BLOCK_SIZE, MFB_ITEM_WIDTH, USERMETA_WIDTH)::type_id::create("m_tx_mfb");
        ...
        m_tx_mvb = uvm_mvb::sequence_lib_tx #(MFB_REGIONS, USERMETA_WIDTH)::type_id::create("m_tx_mvb");
