...
architecture FULL of RATE_LIMITER is
    ...
    constant MI_STATUS_REG_POS       : natural := 0;
    constant MI_SEC_LEN_REG_POS      : natural := 1;
    constant MI_INT_LEN_REG_POS      : natural := 2;
    constant MI_INT_CNT_REG_POS      : natural := 3;
    constant MI_FREQ_REG_POS         : natural := 4;
    constant MI_SPEED_PTR_REG_POS    : natural := 5;
    constant MI_SPEED_REG_POS        : natural := 6;
    constant MI_REGS                 : natural := 7;

    constant SR_IDLE_FLAG            : natural := 0;
    constant SR_CONF_FLAG            : natural := 1;
    constant SR_RUN_FLAG             : natural := 2;
    constant SR_WRITE_AUX_FLAGS_FLAG : natural := 3;
    constant SR_SHAPING_TYPE_FLAG    : natural := 4;

    alias  mi_regs_addr              : std_logic_vector(log2(MI_REGS)-1 downto 0) is MI_ADDR(log2(MI_REGS)+2-1 downto 2);
    signal mi_regs_wren              : std_logic_vector(MI_REGS-1 downto 0);
    signal mi_regs_drd               : slv_array_t(MI_REGS-1 downto 0)(MI_DATA_WIDTH-1 downto 0);
    signal mi_regs_do                : std_logic_vector(MI_DATA_WIDTH-1 downto 0);
    signal mi_regs_dwr               : std_logic_vector(MI_DATA_WIDTH-1 downto 0);

    constant SPEED_PTR_WIDTH         : natural := max(1, log2(INTERVAL_COUNT));

    signal mi_status_reg             : std_logic_vector(MI_DATA_WIDTH-1 downto 0);
    signal mi_sec_len_reg            : std_logic_vector(MI_DATA_WIDTH-1 downto 0);
    signal mi_int_len_reg            : std_logic_vector(MI_DATA_WIDTH-1 downto 0);
    signal mi_speed_regs             : slv_array_t(INTERVAL_COUNT-1 downto 0)(MI_DATA_WIDTH-1 downto 0);
    signal mi_speed_regs_vld         : std_logic_vector(INTERVAL_COUNT-1 downto 0);
    signal mi_speed_regs_wren        : std_logic_vector(INTERVAL_COUNT-1 downto 0);
    signal mi_speed_regs_ptr         : std_logic_vector(SPEED_PTR_WIDTH-1 downto 0);
    signal mi_speed_regs_vld_ptr     : std_logic_vector(SPEED_PTR_WIDTH-1 downto 0);
    signal mi_speed_reg_active       : std_logic_vector(MI_DATA_WIDTH-1 downto 0);
    ...
    signal sec_len_cnt               : std_logic_vector(MI_DATA_WIDTH-1 downto 0);
    signal int_len_cnt               : std_logic_vector(MI_DATA_WIDTH-1 downto 0);
    signal end_of_sec                : std_logic;
    signal new_section               : std_logic;
    signal end_of_int                : std_logic;

    signal active_speed              : unsigned(MI_DATA_WIDTH-1-1 downto 0);
    signal bytes_in_sec              : std_logic_vector(MI_DATA_WIDTH-1 downto 0);
    signal eofs_in_sec               : std_logic_vector(MI_DATA_WIDTH-1 downto 0);
...
begin
    ...
    mi_regs_wren_dec_i : entity work.DEC1FN_ENABLE
    generic map (
        ITEMS  => MI_REGS
    )
    port map (
    ...
    mi_regs_dwr_g: for i in 0 to MI_DATA_WIDTH/8-1 generate
        mi_regs_dwr((i+1)*8-1 downto i*8) <= MI_DWR((i+1)*8-1 downto i*8) when MI_BE(i) = '1' else
                                             mi_regs_do((i+1)*8-1 downto i*8);
    end generate;

    mi_regs_drd(MI_SPEED_REG_POS)     <= mi_speed_reg_active;
    mi_regs_drd(MI_SPEED_PTR_REG_POS) <= resize(mi_speed_regs_ptr, MI_DATA_WIDTH);
    mi_regs_drd(MI_FREQ_REG_POS)      <= std_logic_vector(to_unsigned(FREQUENCY, MI_DATA_WIDTH));
    mi_regs_drd(MI_INT_CNT_REG_POS)   <= std_logic_vector(to_unsigned(INTERVAL_COUNT, MI_DATA_WIDTH));
    mi_regs_drd(MI_INT_LEN_REG_POS)   <= mi_int_len_reg;
    mi_regs_drd(MI_SEC_LEN_REG_POS)   <= mi_sec_len_reg;
    mi_regs_drd(MI_STATUS_REG_POS)    <= mi_status_reg;
    ...
    port map (
        DATA_IN  => slv_array_ser(mi_regs_drd),
        SEL      => mi_regs_addr,
        DATA_OUT => mi_regs_do
    );
    MI_DRD <= mi_regs_do;
    ...
    write_ctrl_flag <= mi_regs_wren(MI_STATUS_REG_POS) and not mi_regs_dwr(SR_WRITE_AUX_FLAGS_FLAG);
    write_aux_flag  <= mi_regs_wren(MI_STATUS_REG_POS) and     mi_regs_dwr(SR_WRITE_AUX_FLAGS_FLAG);
    -- state control flags
    start_conf_flag <= write_ctrl_flag and     mi_regs_dwr(SR_CONF_FLAG);
    stop_conf_flag  <= write_ctrl_flag and not mi_regs_dwr(SR_CONF_FLAG);
    start_run_flag  <= write_ctrl_flag and     mi_regs_dwr(SR_RUN_FLAG);
    stop_run_flag   <= write_ctrl_flag and not mi_regs_dwr(SR_RUN_FLAG);
    -- auxiliary flags
    limit_byte_flag <= write_aux_flag and not mi_regs_dwr(SR_SHAPING_TYPE_FLAG);
    limit_pkts_flag <= write_aux_flag and     mi_regs_dwr(SR_SHAPING_TYPE_FLAG);
    ...
    begin
        if (rising_edge(CLK)) then
            if (RESET = '1') then
                mi_status_reg <= (SR_IDLE_FLAG => '1', others => '0');
            else
    ...
    begin
        if (rising_edge(CLK)) then
            if (RESET = '1') then
                mi_sec_len_reg <= std_logic_vector(to_unsigned(SECTION_LENGTH, MI_DATA_WIDTH));
            elsif (p_state = CONF and mi_regs_wren(MI_SEC_LEN_REG_POS) = '1') then
    ...
    begin
        if (rising_edge(CLK)) then
            if (RESET = '1') then
                mi_int_len_reg <= std_logic_vector(to_unsigned(INTERVAL_LENGTH, MI_DATA_WIDTH));
            elsif (p_state = CONF and mi_regs_wren(MI_INT_LEN_REG_POS) = '1') then
    ...
    begin
        if (rising_edge(CLK)) then
            ...
            elsif (p_state = CONF and mi_regs_wren(MI_SPEED_PTR_REG_POS) = '1') then
                mi_speed_regs_ptr <= mi_regs_dwr(SPEED_PTR_WIDTH-1 downto 0);
            elsif (p_state = RUN and next_speed_req = '1') then
                if (next_speed_vld = '0') then
                    mi_speed_regs_ptr <= (others => '0');
                else
    ...
    -- speed registers
    mi_speed_regs_g : for i in 0 to INTERVAL_COUNT-1 generate
        constant VLD_INIT : std_logic := tsel(i = 0, '1', '0');
    begin
        mi_speed_reg_p : process (CLK)
        begin
            if (rising_edge(CLK)) then
                if (RESET = '1' or start_conf_flag = '1') then
                    mi_speed_regs(i) <= VLD_INIT & std_logic_vector(to_unsigned(OUTPUT_SPEED, MI_DATA_WIDTH-1));
                elsif (mi_speed_regs_wren(i) = '1') then
                    mi_speed_regs(i) <= '1' & mi_regs_dwr(MI_DATA_WIDTH-1-1 downto 0);
                end if;
            end if;
        end process;
        mi_speed_regs_vld(i) <= mi_speed_regs(i)(MI_DATA_WIDTH-1);
    end generate;
    ...
    port map (
        DATA_IN     => mi_speed_regs_vld,
        SEL         => mi_speed_regs_vld_ptr,
        DATA_OUT(0) => next_speed_vld
    );

    mi_speed_regs_wren_demux_i : entity work.GEN_DEMUX
    generic map (
        DATA_WIDTH  => 1,
        DEMUX_WIDTH => INTERVAL_COUNT
    )
    port map (
        DATA_IN(0)  => mi_regs_wren(MI_SPEED_REG_POS),
        SEL         => mi_speed_regs_ptr,
        DATA_OUT    => mi_speed_regs_wren
    );
    ...
    active_speed   <= unsigned(mi_speed_reg_active(MI_DATA_WIDTH-1-1 downto 0));
    ...
    last_eof     <= '1' when active_speed <= unsigned(eofs_in_sec) + LATENCY_RESERVE_PKTS else '0';
    ...
    bytes_over <= '1' when active_speed <= unsigned(bytes_in_sec) + LATENCY_RESERVE_BYTES else '0';
