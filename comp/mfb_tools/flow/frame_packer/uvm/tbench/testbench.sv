...
module testbench;
    ...
    mvb_if #(
        .ITEMS      (MVB_ITEMS),
        .ITEM_WIDTH ($clog2(RX_CHANNELS) + $clog2(USR_RX_PKT_SIZE_MAX+1))
    ) mvb_rx(CLK);
    mvb_if #(
        .ITEMS      (MVB_ITEMS),
        .ITEM_WIDTH ($clog2(RX_CHANNELS) + $clog2(USR_RX_PKT_SIZE_MAX+1) + HDR_META_WIDTH + 1)
    ) mvb_tx(CLK);

    mvb_if #(
        .ITEMS      (1),
        .ITEM_WIDTH (2)
    ) mvb_flow_ctrl[RX_CHANNELS](CLK);
    ...
    initial begin
        automatic virtual mvb_if #(
            .ITEMS      (1),
            .ITEM_WIDTH (2)
        ) vif_mvb_flow_ctrl[RX_CHANNELS] = mvb_flow_ctrl;
        uvm_root m_root;
        ...
        for (int unsigned it = 0; it < RX_CHANNELS; it++) begin
            uvm_config_db#(virtual mvb_if #(
                .ITEMS      (1),
                .ITEM_WIDTH (2)
            ))::set(null, "", $sformatf("vif_mvb_flow_ctrl_%0d", it), vif_mvb_flow_ctrl[it]);
        end
