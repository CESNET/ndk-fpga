...
entity PCIE_PKT_READER is
    generic (
        ...
        -- Instatniate "fake" PCIe pkt reader when True.
        -- Fake variant just generates empty packets of given length while forwarding pkt IDs (in-order only).
        FAKE_READER           : boolean := False;
...
begin
    ...
    fake_reader_g : if not FAKE_READER generate
        ...
        -- ========================================================
        --  Generate MFB packets of given length
        -- ========================================================

        -- Tie off unused PCIe upstream interface (no read requests sent).
        PCIE_UP_MVB_DATA      <= (others => '0');
        PCIE_UP_MVB_VLD       <= (others => '0');
        PCIE_UP_MVB_SRC_RDY   <= '0';

        -- Tie off unused PCIe downstream interface (no completions expected).
        PCIE_DOWN_MVB_DST_RDY <= '1';
        PCIE_DOWN_MFB_DST_RDY <= '1';

        fake_pkt_gen_i : entity work.MFB_USER_PACKET_GEN
        generic map (
            REGIONS        => REGIONS,
            ...
            META_WIDTH     => ID_WIDTH,
            LEN_WIDTH      => log2(PKT_MTU+1),
            FIFO_DEPTH     => 16,
            SHARED_REGIONS => False,
            DEVICE         => DEVICE
        )
        port map (
            CLK         => CLK,
            ...
            GEN_DATA    => USER_REQ_MVB_LENGTH,
            GEN_META    => USER_REQ_MVB_ID,
            GEN_VLD     => USER_REQ_MVB_VLD,
            GEN_SRC_RDY => USER_REQ_MVB_SRC_RDY,
            GEN_DST_RDY => USER_REQ_MVB_DST_RDY,

            TX_DATA     => USER_RESP_MFB_DATA,
            TX_META     => USER_RESP_MFB_ID,
            TX_SOF_POS  => USER_RESP_MFB_SOF_POS,
            TX_EOF_POS  => USER_RESP_MFB_EOF_POS,
            TX_SOF      => USER_RESP_MFB_SOF,
            TX_EOF      => USER_RESP_MFB_EOF,
            TX_SRC_RDY  => USER_RESP_MFB_SRC_RDY,
            TX_DST_RDY  => USER_RESP_MFB_DST_RDY
        );
