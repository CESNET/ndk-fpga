...
class env #(MFB_REGIONS, MFB_REGION_SIZE, MFB_BLOCK_SIZE, MFB_ITEM_WIDTH, DEVICE, ENDPOINT_TYPE) extends uvm_env;
    `uvm_component_param_utils(
        uvm_pcie_cq::env #(MFB_REGIONS, MFB_REGION_SIZE, MFB_BLOCK_SIZE, MFB_ITEM_WIDTH, DEVICE, ENDPOINT_TYPE));
    ...
    function void build_phase(uvm_phase phase);
        ...
        m_env_rx = uvm_logic_vector_array_mfb::env_rx #(MFB_REGIONS, MFB_REGION_SIZE, MFB_BLOCK_SIZE, MFB_ITEM_WIDTH,
                                                       sv_pcie_meta_pack::PCIE_CQ_META_WIDTH)::type_id::create(
            "m_env_rx", this);
    ...
    virtual task run_phase(uvm_phase phase);
        if (m_config.active == UVM_ACTIVE) begin
            uvm_pcie_cq::logic_vector_sequence#(
                sv_pcie_meta_pack::PCIE_CQ_META_WIDTH,
                DEVICE,
                ENDPOINT_TYPE
            ) logic_vector_seq;
            ...
            logic_vector_seq                 = uvm_pcie_cq::logic_vector_sequence#(
                sv_pcie_meta_pack::PCIE_CQ_META_WIDTH,
                DEVICE,
                ENDPOINT_TYPE
            )::type_id::create("logic_vector_seq", this);
            logic_vector_array_seq           = uvm_pcie_cq::logic_vector_array_sequence #(
                MFB_ITEM_WIDTH,
                DEVICE,
                ENDPOINT_TYPE
            )::type_id::create("logic_vector_array_seq", this);
