...
class virt_seq_slow_dma_down #(
    MRRS,
    MIN_READ_REQ_SIZE,
    MPS,
    MIN_WRITE_REQ_SIZE
) extends virt_seq #(MRRS, MIN_READ_REQ_SIZE, MPS, MIN_WRITE_REQ_SIZE);
...
class sequence_stop_var_tx #(
    ...
    TR_MIN,
    TR_MAX
) extends uvm_mfb::sequence_stop_tx #(REGIONS, REGION_SIZE, BLOCK_SIZE, ITEM_WIDTH, META_WIDTH);
    `uvm_object_param_utils(
        test::sequence_stop_var_tx #(REGIONS, REGION_SIZE, BLOCK_SIZE, ITEM_WIDTH, META_WIDTH, TR_MIN, TR_MAX))
...
class sequence_mvb_stop_var_tx #(
    ITEMS,
    ITEM_WIDTH,
    TR_MIN,
    TR_MAX
) extends uvm_mvb::sequence_stop_tx #(ITEMS, ITEM_WIDTH);
...
class mfb_tx_slow #(
    ...
    TR_MIN,
    TR_MAX
) extends uvm_mfb::sequence_lib_tx #(MFB_REGIONS, MFB_REGION_SIZE, MFB_ITEM_WIDTH, MFB_BLOCK_SIZE, META_WIDTH);
    `uvm_object_param_utils(
        test::mfb_tx_slow #(MFB_REGIONS, MFB_REGION_SIZE, MFB_ITEM_WIDTH, MFB_BLOCK_SIZE, META_WIDTH, TR_MIN, TR_MAX))
    ...
    virtual function void init_sequence(uvm_mfb::config_sequence param_cfg = null);
        ...
        this.add_sequence(
            uvm_mfb::sequence_simple_tx #(REGIONS, REGION_SIZE, BLOCK_SIZE, ITEM_WIDTH, META_WIDTH)::get_type());
        this.add_sequence(
            test::sequence_stop_var_tx
                #(REGIONS, REGION_SIZE, BLOCK_SIZE, ITEM_WIDTH, META_WIDTH, TR_MIN, TR_MAX)::get_type());
    endfunction
endclass
...
class mvb_tx_slow #(
    ...
    TR_MIN,
    TR_MAX
) extends uvm_mvb::sequence_lib_tx #(MVB_ITEMS, MVB_DATA_WIDTH);
...
class slow_dma_down_test extends uvm_test;
    ...
    virtual task rq_seq;
        ...
        rq_mfb_lib = uvm_mfb::sequence_lib_tx#(
            MFB_UP_REGIONS,
            MFB_UP_REG_SIZE,
            MFB_UP_BLOCK_SIZE,
            32,
            0
        )::type_id::create("rq_mfb_lib", this);
        rq_axi_lib = uvm_axi::sequence_lib_tx #(
            RQ_TDATA_WIDTH,
            RQ_TUSER_WIDTH,
            MFB_UP_REGIONS
        )::type_id::create("rq_axi_lib", this);
    ...
    virtual task down_seq(int unsigned index);
        test::mfb_tx_slow #(DMA_MFB_DOWN_REGIONS, MFB_DOWN_REG_SIZE, MFB_DOWN_BLOCK_SIZE, MFB_DOWN_ITEM_WIDTH,
                            META_WIDTH, TR_MIN, TR_MAX) down_seq;
        down_seq = test::mfb_tx_slow #(
            DMA_MFB_DOWN_REGIONS,
            MFB_DOWN_REG_SIZE,
            MFB_DOWN_BLOCK_SIZE,
            MFB_DOWN_ITEM_WIDTH,
            ...
            TR_MIN,
            TR_MAX
        )::type_id::create($sformatf("mfb_tx_slow%0d", index));
    ...
    virtual task down_mvb_seq(int unsigned index);
        ...
        down_mvb_seq = test::mvb_tx_slow #(DMA_MVB_DOWN_ITEMS, sv_dma_bus_pack::DMA_DOWNHDR_WIDTH, TR_MIN,
                                          TR_MAX)::type_id::create($sformatf("mvb_tx_slow%0d", index));
    ...
    task run_seq(int unsigned index);
        ...
        m_vseq = virt_seq_slow_dma_down #(MRRS, MIN_READ_REQ_SIZE, MPS, MIN_WRITE_REQ_SIZE)::type_id::create(
            $sformatf("m_vseq%0d", index));
        m_vseq.randomize();
