...
module testbench;
    ...
    )                rx_mvb_vif [RX_STREAMS -1 : 0] (CLK);
    mvb_if #(
        .ITEMS      (MVB_ITEMS*RX_STREAMS),
    ...
    initial begin
        ...
        for (int port = 0; port < RX_STREAMS; port++) begin
            uvm_config_db #(virtual mvb_if #(
                .ITEMS      (MVB_ITEMS),
                .ITEM_WIDTH (MVB_ITEM_WIDTH)
            ))            ::set(null, "", $sformatf("rx_mvb_vif_%0d", port),  v_rx_mvb_vif[port]);
        end

        uvm_config_db #(virtual mvb_if #(
            .ITEMS      (MVB_ITEMS*RX_STREAMS),
            .ITEM_WIDTH ($clog2(RX_STREAMS))
        )) ::set(null, "", "rx_sel_mvb_vif",                   rx_sel_mvb_vif);
        uvm_config_db #(virtual mvb_if #(
            .ITEMS      (MVB_ITEMS*RX_STREAMS),
            .ITEM_WIDTH (MVB_ITEM_WIDTH)
        ))     ::set(null, "", "tx_mvb_vif",                       tx_mvb_vif);
