...
architecture FULL of USP_PCS_PMA_WRAPPER is
    ...
    component ten_gig_eth_pcspma is
        port (
    ...
    component twenty_five_gig_eth_pcspma is
        port (
            GT_RXP_IN_0                         : in STD_LOGIC;
            GT_RXP_IN_1                         : in STD_LOGIC;
            GT_RXP_IN_2                         : in STD_LOGIC;
...
begin
    ...
    gen_10ge: if (not ETH_25G) generate
        ...
        eth10g_i : component ten_gig_eth_pcspma
        port map (
            sys_reset                 => RESET,
            dclk                      => SYSCLK,
            --
    ...
    gen_25ge: if ETH_25G generate
        ...
        eth25g_i : component twenty_five_gig_eth_pcspma
        port map (
            sys_reset     => RESET,
            dclk          => SYSCLK,
            --
    ...
    begin
        ...
        port map (
            ADATAIN  => ber_cnt_clr(i),
            BCLK     => ber_cntr_clk(i),
            BDATAOUT => ber_cnt_clr_sync(i)
        );
        ...
        port map (
            ADATAIN  => blk_err_clr(i),
            BCLK     => blk_cntr_clk(i),
            BDATAOUT => blk_err_clr_sync(i)
        );
        ...
        port map (
            ADATAIN  => gt_rxpolarity_async(i),
            BCLK     => eth0_core_clk(i),
            BRST     => '0',
            BDATAOUT => gt_rxpolarity(i)
        );
        ...
        port map (
            ADATAIN  => gt_txpolarity_async(i),
            BCLK     => eth0_clk(i),
            BRST     => '0',
            BDATAOUT => gt_txpolarity(i)
        );
        ...
        port map (
            ADATAIN  => link_status(i),
            BCLK     => eth0_core_clk(i),
            BDATAOUT => link_status_sync(i)
        );
