-- SPDX-License-Identifier: BSD-3-Clause
-- Copyright (C) 2025 CESNET z. s. p. o
-- Author(s): Ondrej Schwarz <ondrejschwarz@cesnet.cz>

library IEEE;
use IEEE.std_logic_1164.all;
use IEEE.numeric_std.all;
use work.math_pack.all;
use work.type_pack.all;

-- FW implementation of void ShortEnd(uint64 &h0, uint64 &h1, uint64 &h2, uint64 &h3).
entity SPOOKY_SHORTEND is
    generic (
        -- width of the passthrough metadata
        META_WIDTH : natural := 32;
        -- setup of the registers of this component
        REG_SETUP  : std_logic_vector(22-1 downto 0) := "1111111111111111111111"
    );
    port (
        -- main clock
        CLK        : in std_logic;
        -- synchronious reset
        RESET      : in std_logic;

        -- variables used in hash calculation
        IN_H0      : in  unsigned(64-1 downto 0);
        IN_H1      : in  unsigned(64-1 downto 0);
        IN_H2      : in  unsigned(64-1 downto 0);
        IN_H3      : in  unsigned(64-1 downto 0);
        -- metadata input
        IN_META    : in  std_logic_vector(META_WIDTH-1 downto 0);
        -- validity of input
        IN_VALID   : in  std_logic;

        -- updated variables
        OUT_H0     : out unsigned(64-1 downto 0);
        OUT_H1     : out unsigned(64-1 downto 0);
        OUT_H2     : out unsigned(64-1 downto 0);
        OUT_H3     : out unsigned(64-1 downto 0);
        -- metadata output
        OUT_META   : out std_logic_vector(META_WIDTH-1 downto 0);
        -- valid passthrough
        OUT_VALID  : out std_logic
    );
end entity;

architecture FULL of SPOOKY_SHORTEND is
    -- length of the pipeline of this component
    constant PIPE_LENGTH : integer := 22;

    signal h0     : u_array_t(PIPE_LENGTH-1 downto 0)(64-1 downto 0);
    signal h1     : u_array_t(PIPE_LENGTH-1 downto 0)(64-1 downto 0);
    signal h2     : u_array_t(PIPE_LENGTH-1 downto 0)(64-1 downto 0);
    signal h3     : u_array_t(PIPE_LENGTH-1 downto 0)(64-1 downto 0);

    signal h0_reg : u_array_t(PIPE_LENGTH-1 downto 0)(64-1 downto 0);
    signal h1_reg : u_array_t(PIPE_LENGTH-1 downto 0)(64-1 downto 0);
    signal h2_reg : u_array_t(PIPE_LENGTH-1 downto 0)(64-1 downto 0);
    signal h3_reg : u_array_t(PIPE_LENGTH-1 downto 0)(64-1 downto 0);

    signal vld    : std_logic_vector(PIPE_LENGTH-1 downto 0);
    signal meta   : slv_array_t(PIPE_LENGTH-1 downto 0)(META_WIDTH-1 downto 0);

begin
    -- ================================================
    --                      INPUT
    -- ================================================
    h0_reg(21) <= IN_H0;
    h1_reg(21) <= IN_H1;
    h2_reg(21) <= IN_H2;
    h3_reg(21) <= IN_H3;
    vld(21)    <= IN_VALID;
    meta(21)   <= IN_META;

    -- ================================================
    --                     LOGIC
    -- ================================================

    -- h3 ^= h2;  h2 = Rot64(h2,15); h3 += h2;
    h0(21) <= h0_reg(21);
    h1(21) <= h1_reg(21);
    h2(21) <= rotate_left(h2_reg(21), 15);
    h3(21) <= h3_reg(21) xor h2_reg(21);

    h0(20) <= h0_reg(20);
    h1(20) <= h1_reg(20);
    h2(20) <= h2_reg(20);
    h3(20) <= h3_reg(20) + h2_reg(20);

    -- h0 ^= h3;  h3 = Rot64(h3,52);  h0 += h3;
    h0(19) <= h0_reg(19) xor h3_reg(19);
    h1(19) <= h1_reg(19);
    h2(19) <= h2_reg(19);
    h3(19) <= rotate_left(h3_reg(19), 52);

    h0(18) <= h0_reg(18) + h3_reg(18);
    h1(18) <= h1_reg(18);
    h2(18) <= h2_reg(18);
    h3(18) <= h3_reg(18);

    -- h1 ^= h0;  h0 = Rot64(h0,26);  h1 += h0;
    h0(17) <= rotate_left(h0_reg(17), 26);
    h1(17) <= h1_reg(17) xor h0_reg(17);
    h2(17) <= h2_reg(17);
    h3(17) <= h3_reg(17);

    h0(16) <= h0_reg(16);
    h1(16) <= h1_reg(16) + h0_reg(16);
    h2(16) <= h2_reg(16);
    h3(16) <= h3_reg(16);

    -- h2 ^= h1;  h1 = Rot64(h1,51);  h2 += h1;
    h0(15) <= h0_reg(15);
    h1(15) <= rotate_left(h1_reg(15), 51);
    h2(15) <= h2_reg(15) xor h1_reg(15);
    h3(15) <= h3_reg(15);

    h0(14) <= h0_reg(14);
    h1(14) <= h1_reg(14);
    h2(14) <= h2_reg(14) + h1_reg(14);
    h3(14) <= h3_reg(14);

    -- h3 ^= h2;  h2 = Rot64(h2,28);  h3 += h2;
    h0(13) <= h0_reg(13);
    h1(13) <= h1_reg(13);
    h2(13) <= rotate_left(h2_reg(13), 28);
    h3(13) <= h3_reg(13) xor h2_reg(13);

    h0(12) <= h0_reg(12);
    h1(12) <= h1_reg(12);
    h2(12) <= h2_reg(12);
    h3(12) <= h3_reg(12) + h2_reg(12);

    -- h0 ^= h3;  h3 = Rot64(h3,9);   h0 += h3;
    h0(11) <= h0_reg(11) xor h3_reg(11);
    h1(11) <= h1_reg(11);
    h2(11) <= h2_reg(11);
    h3(11) <= rotate_left(h3_reg(11), 9);

    h0(10) <= h0_reg(10) + h3_reg(10);
    h1(10) <= h1_reg(10);
    h2(10) <= h2_reg(10);
    h3(10) <= h3_reg(10);

    -- h1 ^= h0;  h0 = Rot64(h0,47);  h1 += h0;
    h0(9) <= rotate_left(h0_reg(9), 47);
    h1(9) <= h1_reg(9) xor h0_reg(9);
    h2(9) <= h2_reg(9);
    h3(9) <= h3_reg(9);

    h0(8) <= h0_reg(8);
    h1(8) <= h1_reg(8) + h0_reg(8);
    h2(8) <= h2_reg(8);
    h3(8) <= h3_reg(8);

    -- h2 ^= h1;  h1 = Rot64(h1,54);  h2 += h1;
    h0(7) <= h0_reg(7);
    h1(7) <= rotate_left(h1_reg(7), 54);
    h2(7) <= h2_reg(7) xor h1_reg(7);
    h3(7) <= h3_reg(7);

    h0(6) <= h0_reg(6);
    h1(6) <= h1_reg(6);
    h2(6) <= h2_reg(6) + h1_reg(6);
    h3(6) <= h3_reg(6);

    -- h3 ^= h2;  h2 = Rot64(h2,32);  h3 += h2;
    h0(5) <= h0_reg(5);
    h1(5) <= h1_reg(5);
    h2(5) <= rotate_left(h2_reg(5), 32);
    h3(5) <= h3_reg(5) xor h2_reg(5);

    h0(4) <= h0_reg(4);
    h1(4) <= h1_reg(4);
    h2(4) <= h2_reg(4);
    h3(4) <= h3_reg(4) + h2_reg(4);

    -- h0 ^= h3;  h3 = Rot64(h3,25);  h0 += h3;
    h0(3) <= h0_reg(3) xor h3_reg(3);
    h1(3) <= h1_reg(3);
    h2(3) <= h2_reg(3);
    h3(3) <= rotate_left(h3_reg(3), 25);

    h0(2) <= h0_reg(2) + h3_reg(2);
    h1(2) <= h1_reg(2);
    h2(2) <= h2_reg(2);
    h3(2) <= h3_reg(2);

    -- h1 ^= h0;  h0 = Rot64(h0,63);  h1 += h0;
    h0(1) <= rotate_left(h0_reg(1), 63);
    h1(1) <= h1_reg(1) xor h0_reg(1);
    h2(1) <= h2_reg(1);
    h3(1) <= h3_reg(1);

    h0(0) <= h0_reg(0);
    h1(0) <= h1_reg(0) + h0_reg(0);
    h2(0) <= h2_reg(0);
    h3(0) <= h3_reg(0);

    -- ================================================
    --                    PIPELINE
    -- ================================================
    pipeline_g: for g in PIPE_LENGTH-1 downto 1 generate
        reg_g: if REG_SETUP(g) = '1' generate
            process (CLK)
            begin
                if rising_edge(CLK) then
                    h0_reg(g-1)  <= h0(g);
                    h1_reg(g-1)  <= h1(g);
                    h2_reg(g-1)  <= h2(g);
                    h3_reg(g-1)  <= h3(g);
                    meta(g-1)    <= meta(g);
                    vld(g-1)     <= vld(g);

                    if (RESET = '1') then
                        vld(g-1) <= '0';
                    end if;
                end if;
            end process;
        else generate
            h0_reg(g-1)  <= h0(g);
            h1_reg(g-1)  <= h1(g);
            h2_reg(g-1)  <= h2(g);
            h3_reg(g-1)  <= h3(g);
            meta(g-1)    <= meta(g);
            vld(g-1)     <= vld(g);
        end generate;
    end generate;

    -- ================================================
    --                     OUTPUT
    -- ================================================
    out_reg_g: if REG_SETUP(0) = '1' generate
        process (CLK)
        begin
            if rising_edge(CLK) then
                OUT_H0    <= h0(0);
                OUT_H1    <= h1(0);
                OUT_H2    <= h2(0);
                OUT_H3    <= h3(0);
                OUT_META  <= meta(0);
                OUT_VALID <= vld(0);

                if (RESET = '1') then
                    OUT_VALID <= '0';
                end if;
            end if;
        end process;
    else generate
        OUT_H0    <= h0(0);
        OUT_H1    <= h1(0);
        OUT_H2    <= h2(0);
        OUT_H3    <= h3(0);
        OUT_META  <= meta(0);
        OUT_VALID <= vld(0);
    end generate;

end architecture;
