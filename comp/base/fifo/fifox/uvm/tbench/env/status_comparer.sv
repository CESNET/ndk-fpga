...
class status_comparer #(
    int unsigned STATUS_WIDTH
) extends uvm_common::comparer_ordered #(
    uvm_logic_vector::sequence_item #(STATUS_WIDTH+2)
);
    ...
    function string model_item2string(uvm_logic_vector::sequence_item #(STATUS_WIDTH+2) tr);
        ...
        msg = $sformatf("\nStatus\n%s\n\tITEMS %0d\n\tAlmoust full 0b%b\n\tAlmoust empty 0b%b",
                    tr.time2string(), status, afull, aempty
        );
