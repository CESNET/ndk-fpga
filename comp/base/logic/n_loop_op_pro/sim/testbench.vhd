    ...
    begin
        ...
        for i in 0 to READ_PORTS-1 loop
            if (read_data(i) /= fake_mem(to_integer(unsigned(read_addr_reg1(i))))) then
                test_status <= '0';
                report "data: " & integer'image(to_integer(unsigned(read_data(i)))) & " addr_reg1:" & integer'image(to_integer(unsigned(read_addr_reg1(i)))) & " fake_mem_data:" & integer'image(to_integer(unsigned(fake_mem(to_integer(unsigned(read_addr_reg1(i)))))))
                    severity error;
