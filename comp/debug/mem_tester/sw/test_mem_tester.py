...
def test_config():
    for i in range(0, comp_cnt()):
        tester = open(i)
        config = tester.mem_logger.stats['Constants']
...
def test_seq():
    for i in range(0, comp_cnt()):
        ...
        min_burst = 1
        max_burst = 2 ** config["MEM_BURST_WIDTH"] - 1
        ...
        for b in (min_burst, max_burst):
            ...
            assert sum(stats['Requests']['wr req cnt']) > 0, "No write requests were send"
            assert sum(stats['Requests']['rd req cnt']) > 0, "No read requests were send"
