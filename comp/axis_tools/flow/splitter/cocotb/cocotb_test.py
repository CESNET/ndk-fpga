...
from cocotbext.ofm.axi4stream.protocol import Axi4StreamProtocol, optional_signal, parameter
...
class Axi4StreamProtocolWithSel(Axi4StreamProtocol):
    @parameter
    def tx_streams(self):
        return self.dut.TX_STREAMS.value

    SEL: LogicArray = optional_signal(put_with="TVALID")
...
class testbench():
    def __init__(self, dut, debug=False):
        ...
        self.stream_in: Axi4StreamMaster  = Axi4StreamMaster(dut, "RX_AXIS", dut.CLK, protocol=Axi4StreamProtocolWithSel, rate_limiter_config=rate_limiter_config)
        ...
        for i in range(self.stream_in.bus.tx_streams):
...
async def run_test(dut, min_size=4, max_size=512, pkt_count=10000):
    ...
    for transaction in random_transactions(Axi4StreamTransaction, tb.stream_in, "TDATA", min_size, max_size, pkt_count):
        selector = randint(0, tb.stream_in.bus.tx_streams-1)
