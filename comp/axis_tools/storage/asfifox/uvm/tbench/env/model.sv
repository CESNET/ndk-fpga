...
class model #(
    ...
    `uvm_component_utils(uvm_asfifox::model #(ITEM_WIDTH))

    // port for input data
    uvm_tlm_analysis_fifo#(uvm_logic_vector_array::sequence_item#(ITEM_WIDTH)) m_rx;
    // port for output data
    uvm_analysis_port    #(uvm_logic_vector_array::sequence_item#(ITEM_WIDTH)) m_tx;
