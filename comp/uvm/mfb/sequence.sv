...
class sequence_simple_tx #(
    ...
    #(config_sequence, sequence_item #(REGIONS, REGION_SIZE, BLOCK_SIZE, ITEM_WIDTH, META_WIDTH));
    `ndk_object_param_utils(
        uvm_mfb::sequence_simple_tx#(REGIONS, REGION_SIZE, BLOCK_SIZE, ITEM_WIDTH, META_WIDTH),
                            $sformatf("uvm_mfb::sequence_simple_tx #(%0d,%0d,%0d,%0d,%0d)", REGIONS, REGION_SIZE,
...
class sequence_full_speed_tx #(
    ...
    `ndk_object_param_utils(
        uvm_mfb::sequence_full_speed_tx#(REGIONS, REGION_SIZE, BLOCK_SIZE, ITEM_WIDTH, META_WIDTH),
                            $sformatf("uvm_mfb::sequence_full_speed_tx #(%0d,%0d,%0d,%0d,%0d)", REGIONS, REGION_SIZE,
                                      BLOCK_SIZE, ITEM_WIDTH, META_WIDTH)
    )
...
class sequence_stop_tx #(
    ...
    `ndk_object_param_utils(
        uvm_mfb::sequence_stop_tx#(REGIONS, REGION_SIZE, BLOCK_SIZE, ITEM_WIDTH, META_WIDTH),
                            $sformatf("uvm_mfb::sequence_stop_tx #(%0d,%0d,%0d,%0d,%0d)", REGIONS, REGION_SIZE,
                                      BLOCK_SIZE, ITEM_WIDTH, META_WIDTH)
    )
...
class sequence_lib_tx #(
  ...
  `ndk_object_param_utils(
        uvm_mfb::sequence_lib_tx#(REGIONS, REGION_SIZE, BLOCK_SIZE, ITEM_WIDTH, META_WIDTH),
                            $sformatf("uvm_mfb::sequence_lib_tx #(%0d,%0d,%0d,%0d,%0d)", REGIONS, REGION_SIZE,
                                      BLOCK_SIZE, ITEM_WIDTH, META_WIDTH)
    )
...
class sequence_lib_tx_speed #(
    ...
    int unsigned ITEM_WIDTH,
    int unsigned META_WIDTH
) extends sequence_lib_tx #(REGIONS, REGION_SIZE, BLOCK_SIZE, ITEM_WIDTH, META_WIDTH);
  `ndk_object_param_utils(
        uvm_mfb::sequence_lib_tx_speed#(REGIONS, REGION_SIZE, BLOCK_SIZE, ITEM_WIDTH, META_WIDTH),
                            $sformatf("uvm_mfb::sequence_lib_tx_speed #(%0d,%0d,%0d,%0d,%0d)", REGIONS, REGION_SIZE,
                                      BLOCK_SIZE, ITEM_WIDTH, META_WIDTH)
    )
