...
virtual class comparer_base_tagged #(
    type MODEL_ITEM,
    ...
    virtual function void write_dut(DUT_ITEM tr);
        ...
        dut_accept += 1;
        `uvm_info(this.get_full_name(), $sformatf(
                  "\n\tGet DUT transactions %0d in time %0dns\n%s\n", dut_accept, $time() / 1ns, dut_item2string(tr)),
                  UVM_FULL);
        ...
        for (int unsigned it = 0; it < dut_items.size(); it++) begin
            msg = {
                ...
                $sformatf(
                    ...
                    dut_items[it].time_last() / 1ns,
                    (dut_items[it].time_last() - tr_time) / 1ns,
                    model_item2string(
                        tr
                    ),
                    dut_item2string(
    ...
    virtual function string info(logic data = 0);
        ...
        while (index_valid != 0) begin
            msg = {
                msg,
                $sformatf(
                    "\n\tTag %s Errors/Compared %0d/%0d transactions",
                    index,
                    model_items[index].errors,
                    model_items[index].compared
                )
            };
            if (data == 1) begin
                for (int unsigned it = 0; it < model_items[index].size(); it++) begin
                    msg = {
                        msg,
                        $sformatf("\n\tModels transaction : %0d", it),
                        model_item2string(model_items[index].get(it))
                    };
                end
            end
            //next index
